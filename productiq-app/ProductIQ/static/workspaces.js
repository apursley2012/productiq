(() => {
	...
	const price = (r) =>
		Number(
			r.pricing?.suggestedPrice ??
				String(r.price || "").replace(/[^0-9.]/g, ""),
		) || null;
	const title = (r) =>
		r.title || r.sourceInput?.name || r.asin || "Untitled product";
	const img = (r) => r.images?.[0] || "";
	const category = (r) => r.catalogCategory?.category || "Uncategorized";
	const subcategory = (r) =>
		r.catalogCategory?.subcategory || "General Merchandise";
	window.PIQ = {
		$,
		esc,
		money,
