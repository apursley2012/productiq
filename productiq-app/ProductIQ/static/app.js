(() => {
	...
	function upsertResult(result) {
		...
		const index = id
			? state.results.findIndex((existing) => identity(existing) === id)
			: -1;
	...
	async function enrichCatalog() {
		...
		try {
			...
			if (!response.ok)
				throw new Error(data.error || "Catalog analysis failed.");
	...
	function queueItem(value) {
		...
		if (type === "ASIN")
			return { asin: value.toUpperCase(), url: "", name: "" };
		if (type === "UPC/EAN")
			return { upc: value.replace(/[\s-]/g, ""), asin: "", url: "", name: "" };
	...
	function addItems(items) {
		for (const item of items) {
			...
			if (
				!state.queue.some(
					(existing) => label(existing).trim().toLowerCase() === id,
				)
			) {
	...
	function renderQueue() {
      ...
      <tr>
		...
		$("#queue-count").textContent =
			`${state.queue.length} product${state.queue.length === 1 ? "" : "s"}`;
	...
	function fillSelect(id, selected = "") {
		...
		select.innerHTML =
			...
			state.importedColumns
				.map(
					(column) =>
						`<option ${column === selected ? "selected" : ""}>${esc(column)}</option>`,
				)
				.join("");
	}

	$("#add-lines").onclick = () => {
		const lines = $("#product-input")
			.value.split(/\n+/)
	...
	async function loadSampleProducts(trigger) {
		...
		try {
			...
			if (!items.length) {
				...
				const data = await readJson(response);
				if (!response.ok)
					throw new Error(data.error || "Could not load the sample data.");
				items = data.items || [];
			}
	...
	$("#load-sample").onclick = (event) =>
		loadSampleProducts(event.currentTarget);
	$("#hero-load-sample").onclick = (event) =>
		loadSampleProducts(event.currentTarget);
	...
	$("#file-input").onchange = async (event) => {
		...
		try {
			...
			fillSelect(
				"#map-sku",
				guess(data.columns, [/^sku$/i, /item.*sku/i, /seller.*sku/i]),
			);
			fillSelect(
				"#map-upc",
				guess(data.columns, [/^upc$/i, /^ean$/i, /gtin/i, /barcode/i]),
			);
			fillSelect(
				"#map-model",
				guess(data.columns, [/model/i, /mpn/i, /part.*number/i]),
			);
			fillSelect(
				"#map-brand",
				guess(data.columns, [/brand/i, /manufacturer/i]),
			);
			fillSelect(
				"#map-cost",
				guess(data.columns, [/cost/i, /purchase.*price/i, /price.*paid/i]),
			);
			fillSelect(
				"#map-quantity",
				guess(data.columns, [/quantity/i, /^qty$/i, /stock/i, /on.*hand/i]),
			);
			fillSelect(
				"#map-shipping-cost",
				guess(data.columns, [/shipping.*cost/i, /inbound.*shipping/i]),
			);
			fillSelect("#map-fees", guess(data.columns, [/fixed.*fee/i, /^fees?$/i]));
			fillSelect(
				"#map-fee-rate",
				guess(data.columns, [/fee.*rate/i, /fee.*%/i]),
			);
			fillSelect(
				"#map-category",
				guess(data.columns, [/^category$/i, /department/i]),
			);
			fillSelect(
				"#map-subcategory",
				guess(data.columns, [/sub.*category/i, /product.*type/i]),
			);
			fillSelect("#map-condition", guess(data.columns, [/condition/i]));
			fillSelect(
				"#map-pack-count",
				guess(data.columns, [/pack.*count/i, /units.*pack/i, /^count$/i]),
			);

			$("#mapping").classList.remove("hidden");
			$("#upload-message").textContent =
				`Loaded ${data.count} rows. Confirm the column mapping.`;
		} catch (error) {
		...
		const items = state.importedRows
			.map((row) => ({
				...
				quantity: quantityColumn
					? String(row[quantityColumn] || "").trim()
					: "",
				shipping_cost: shippingColumn
					? String(row[shippingColumn] || "").trim()
					: "",
				fees: feesColumn ? String(row[feesColumn] || "").trim() : "",
				fee_rate: feeRateColumn ? String(row[feeRateColumn] || "").trim() : "",
				category: categoryColumn
					? String(row[categoryColumn] || "").trim()
					: "",
				subcategory: subcategoryColumn
					? String(row[subcategoryColumn] || "").trim()
					: "",
				condition: conditionColumn
					? String(row[conditionColumn] || "").trim()
					: "",
				pack_count: packCountColumn
					? String(row[packCountColumn] || "").trim()
					: "",
			}))
			.filter(
				(item) => item.asin || item.url || item.name || item.upc || item.model,
			);
	...
	$("#captcha-cancel").onclick = () => {
		...
		state.activeCaptchaJob = null;
		reject?.(
			new Error("Research stopped at the Amazon verification checkpoint."),
		);
	};
		...
		try {
			...
			while (completed < total) {
				setProgress(
					completed,
					total,
					`Researching ${label(state.queue[completed])}`,
				);
			...
			const message =
				error?.message || String(error) || "Product research failed.";
	...
	async function refreshBrowserActivity(jobId) {
		...
		try {
			const response = await fetch(
				`/api/jobs/${jobId}/browser-debug?ts=${Date.now()}`,
				{ cache: "no-store" },
			);
			const data = await readJson(response);
	...
	function startBrowserPolling(jobId) {
		...
		state.browserPollTimer = setInterval(
			() => refreshBrowserActivity(jobId),
			1200,
		);
	}
	...
	function renderMetrics() {
		...
		$("#metric-complete").textContent = state.results.filter(
			(item) => item.status === "Complete",
		).length;
		$("#metric-review").textContent = state.results.filter(
			(item) => item.status === "Needs review",
		).length;
		$("#metric-error").textContent = state.results.filter(
			(item) => item.status === "Error",
		).length;
		$("#metric-competitors").textContent = state.results.reduce(
			(count, item) => count + (item.competitors?.length || 0),
			0,
		);
	...
	function filteredResults() {
		...
		return state.results.filter((result) => {
			const matchesStatus =
				!state.resultStatus || result.status === state.resultStatus;
			const haystack = [
				result.title,
				result.asin,
				result.brand,
				...
				result.seller,
				result.price,
				result.sourceInput?.upc,
				result.sourceInput?.model,
				result.catalogCategory?.category,
				result.catalogCategory?.subcategory,
			]
				.filter(Boolean)
				.join(" ")
				.toLowerCase();
	...
	function renderResults() {
		...
		$("#retry-incomplete").disabled = !state.results.some(
        ...
        <div>
            ...
            <div class="competitor-list">${result.competitors
              ...
              <p>
                ...
                ${candidate.url ? ` <a href="${esc(candidate.url)}" target="_blank" rel="noopener">View listing</a>` : ""}
							...
							: competitorDiagnostics(result)
					}

          ${
						result.crossSells?.length
							? `<details>
            <summary>Cross-sells from this catalog (${result.crossSells.length})</summary>
            <div class="recommendation-list">${result.crossSells
							.map(
								(item) => `
          ...
          ${
						result.upsells?.length
							? `<details>
            <summary>Upsells from this catalog (${result.upsells.length})</summary>
            <div class="recommendation-list">${result.upsells
							.map(
								(item) => `
              <div class="recommendation-row">
                ${item.image ? `<img src="${esc(item.image)}" alt="">` : ""}
          ...
          <details>
            ...
            ${
							result.details && Object.keys(result.details).length
								? `<h4>Technical details</h4><ul>${Object.entries(
										result.details,
									)
										.map(
											([key, value]) =>
