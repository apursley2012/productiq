document.addEventListener("DOMContentLoaded", () => {
	...
	select.innerHTML = results.length
		? results
				.map(
					(result, index) =>
						`<option value="${index}">${esc(title(result))}</option>`,
				)
				.join("")
	...
	if (wanted) {
		const index = results.findIndex(
			(result) => (result.asin || title(result)) === wanted,
		);
	...
	function render() {
		...
		if (!result) {
			$("#competitor-table").innerHTML =
				'<div class="empty">Research products first.</div>';
			return;
		}
	...
	$("#refresh-competitors").onclick = async () => {
		...
		try {
			...
			result.catalogCategory =
				data.catalogCategory || result.catalogCategory || {};
