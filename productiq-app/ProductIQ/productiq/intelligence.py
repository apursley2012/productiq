...
def _google_results(query: str, timeout=5, limit=18):
    ...
    for anchor in soup.select("div.yuRUbf > a, a[jsname='UWckNb'], div.g a"):
        ...
        title_el = anchor.select_one("h3") or (
            block.select_one("h3") if block else None
        )
        snippet_el = (
            block.select_one("div.VwiC3b, div[data-sncf], span.aCOpRe")
            if block
            else None
        )
        results.append(
...
def _page_listing_data(url: str, timeout=4.5):
    try:
        response = requests.get(
            url, headers=SEARCH_HEADERS, timeout=timeout, allow_redirects=True
        )
        ...
        if price is None:
            visible_prices = [
                _num(x) for x in PRICE_RE.findall(soup.get_text(" ", strip=True)[:9000])
            ]
        ...
        if not title:
            ...
            title = (
                ...
                else (node.get_text(" ", strip=True) if node else "")
            )
        ...
        return {
            ...
            "brand": _brand_value(product.get("brand"))
...
def _variant_conflicts(source_text, listing_text):
    conflicts = []
    source_pack, listing_pack = _extract_pack_count(source_text), _extract_pack_count(
        listing_text
    )
    ...
    if source_size and listing_size and source_size[0] == listing_size[0]:
        delta = abs(source_size[1] - listing_size[1]) / max(
            source_size[1], listing_size[1]
        )
        if delta > 0.03:
            conflicts.append("size/weight differs")
    source_condition, listing_condition = _extract_condition(
        source_text
    ), _extract_condition(listing_text)
    if source_condition != listing_condition:
        conflicts.append(
            f"condition differs ({source_condition} vs {listing_condition})"
        )
    return conflicts
...
def research_competitors(result, source, timeout=5, max_results=15):
    ...
    if not queries:
        result["competitorResearch"] = {
            "queriesTried": [],
            "providers": {},
            "message": "No usable search identifiers.",
        }
        return []
    ...
    for item in raw:
        ...
        initial_text = " ".join(
            [item.get("title", ""), item.get("snippet", ""), item.get("query", "")]
        )
        ...
        confidence, score, reason, conflicts = _match_details(
...
def _derived_subcategory(title):
    ...
    tokens = [
        t for t in text.split() if len(t) >= 3 and t not in stop and not t.isdigit()
    ]
    if not tokens:
        return "General Merchandise"
...
def categorize_product(result, source=None):
    ...
    if explicit_category:
        return {
            ...
            "subcategory": explicit_subcategory
            or (
                amazon_path[-1]
                if amazon_path
                else _derived_subcategory(result.get("title") or source.get("name"))
            ),
    ...
    if best and best[0] > 0:
        ...
        return {
            ...
            "subcategory": best[2],
            "amazonPath": amazon_path,
            "source": "Amazon/schema path + product data"
            if (amazon_path or schema_signals)
            else "Product data",
    ...
    if all_paths:
        broad = (
            _broad_from_path(all_paths)
            or str(all_paths[0]).strip()
            or "General Merchandise"
        )
        leaf = (
            str(all_paths[-1]).strip()
            if len(all_paths) > 1
            else _derived_subcategory(result.get("title") or source.get("name"))
        )
        return {
...
def _available(candidate):
    ...
    return not any(
        ...
        for term in ("out of stock", "unavailable", "currently unavailable")
    )
...
def _cross_score(base, candidate):
    ...
    if csub in COMPLEMENTARY.get(bsub, set()):
        ...
        base_tokens = set(
            _title_tokens(
                base.get("title") or (base.get("sourceInput") or {}).get("name")
            )
        )
        cand_tokens = set(
            _title_tokens(
                candidate.get("title")
                or (candidate.get("sourceInput") or {}).get("name")
            )
        )
    ...
    brand_a = _norm_text(
        base.get("brand") or (base.get("sourceInput") or {}).get("brand")
    )
    brand_b = _norm_text(
        candidate.get("brand") or (candidate.get("sourceInput") or {}).get("brand")
    )
...
def _upsell_score(base, candidate):
    ...
    if (
        ...
        or candidate_price <= base_price * 1.05
    ):
        return -1, ""
...
def enrich_catalog(results, max_cross_sells=8, max_upsells=8):
    for result in results:
        result["catalogCategory"] = categorize_product(
            result, result.get("sourceInput") or {}
        )
...
def catalog_summary(results):
    ...
    quantities = [
        _num((result.get("sourceInput") or {}).get("quantity")) for result in results
    ]
    ...
    return {
        ...
        "averageEstimatedMargin": round(statistics.mean(margins), 1)
        if margins
        else None,
        "productsWithCompetitors": sum(
            1 for result in results if result.get("competitors")
        ),
        "productsNeedingReview": sum(
            1 for result in results if result.get("status") != "Complete"
        ),
        "totalCrossSellSuggestions": sum(
            len(result.get("crossSells") or []) for result in results
        ),
        "totalUpsellSuggestions": sum(
            len(result.get("upsells") or []) for result in results
        ),
    }
...
def _pricing_from_competitors(result, source, competitors):
    ...
    fixed_fees = (
        _num(source.get("fees") or source.get("fixed_fees") or source.get("fixedFees"))
        or 0
    )
    ...
    break_even = (
        round(cost + shipping_cost + fixed_fees, 2) if cost is not None else None
    )
    if break_even is not None and 0 < fee_rate < 100:
        break_even = round(break_even / (1 - fee_rate / 100), 2)

    variable_fees = (
        round((suggested or 0) * fee_rate / 100, 2) if suggested is not None else 0
    )
    profit = (
        round(suggested - cost - shipping_cost - fixed_fees - variable_fees, 2)
        if suggested is not None and cost is not None
        else None
    )
    margin = (
        round((profit / suggested) * 100, 1)
        if profit is not None and suggested
        else None
    )
    ...
    return {
        ...
        "premiumPrice": round(high * 1.05, 2)
        if high is not None
        else (round(suggested * 1.10, 2) if suggested else None),
...
def add_intelligence(result, source, *, research_market=True):
    ...
    competitors = (
        research_competitors(result, source)
        if research_market
        else (result.get("competitors") or [])
    )
    ...
    result["identification"] = {
        "status": (
            "Identifier-backed product"
            if any(identifiers.get(key) for key in ("asin", "upc", "model"))
